...
enum MotorDirection { STOPPED, OPENING, CLOSING };
MotorDirection currentDirection = STOPPED;
void openMotor() {
  if (currentDirection != OPENING) { // prevent redundant reactivation
    digitalWrite(IN1, HIGH);
    digitalWrite(IN2, LOW);
    analogWrite(ENA, 125); // PWM speed (0–255)
...
void autoBridgeControl() {
  if (!autoSequenceRunning) {
    // Detect ship and start sequence
    if ((distance1 > 0 && distance1 < 100) || (distance2 > 0 && distance2 < 100)) {
      shipDetected = true;
    ...
    // Step 1: Open bridge for 3 sec
    if (sequenceStep == 1 && elapsed >= 3000) {
      stopMotor();
      sequenceStep = 2;
      sequenceStartTime = millis();
    }
    // Step 2: Wait 1 sec before closing (optional)
    else if (sequenceStep == 2 && elapsed >= 3000) {
      closeMotor();
      sequenceStep = 3;
      sequenceStartTime = millis();
    }
    // Step 3: Close bridge for 3 sec
    else if (sequenceStep == 3 && elapsed >= 3000) {
      stopMotor();
      sequenceStep = 4;
      sequenceStartTime = millis();
    }
    // Step 4: Sequence done
      ...
      digitalWrite(output26, LOW);   // Red off
      digitalWrite(output27, HIGH);  // Blue on
    }
  }
}
...
void loop() {
  ...
  else if(motorRunning && (millis() - motorStartTime >= 3000)) { // stops the motor after its been running for 3 seconds
    stopMotor();
  }
  // Sensor check
